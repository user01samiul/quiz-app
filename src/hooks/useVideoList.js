//hook for using data from firebase database in many components
...
export default function useVideoList(page) {
  ...
  useEffect(() => {
    ...
    async function fetchVideos() {
      ...
      const videoQuery = query(videosRef, orderByKey(), startAt(""+ page),limitToFirst(6)); //node ref, how will i get data from videos (according to order), start getting data from(string), get first 6 from starting
      ...
      try {
        ...
        if(snapShot.exists()){       //checking if data exists | exists() is a method of get()
            setVideos((prev)=>{
